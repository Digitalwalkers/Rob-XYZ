...
export default function SpeedChart() {
  ...
  // Extract speed per robot from grouped data
  const seriesData = useMemo(() => {
    const map = {};
    for (const { robotId, data } of robotData) {
      map[robotId] = data.map((row) => [new Date(row.timestamp).getTime(), row.speed]);
    }
    return map;
  }, [robotData]);
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          const lines = params.map(
            (p) => `<span style="color:${p.color}">\u25CF</span> ${p.seriesName}: ${p.value[1].toFixed(2)}`
          );
